...
//static const CGFloat CSTToasBottomInset         = 50.0f;
...
// add by qinss start
NSString * const CSToastPositionBigBottom       = @"bigButtom";
// add by qinss end
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        }else if ([point caseInsensitiveCompare:CSToastPositionBigBottom] == NSOrderedSame){
//            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height - 150);
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding - (CGRectGetHeight(self.bounds) / 3 - 80));
        }
    ...
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding - (CGRectGetHeight(self.bounds) / 3));
}
